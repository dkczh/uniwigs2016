	...
	<div class="primary_block row">
		...
		<div class="pb-center-column col-xs-12 col-sm-6">
			...
			<ul class="product-sku" {if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
				...
				{if isset($review_summary) && $review_summary.count>0}
				<li class="price-star fll mr20">
					<a href="javascript:void(0);"  class="product-rating-link" title="{if $review_summary.rating>0}{$review_summary.rating}/5{/if}">
						<span class="star" style="width:{$review_summary.rating/5*100}%"></span>
					</a>
					<a href="#product_reviews" data-uk-smooth-scroll>
						{$review_summary.count} review{if $review_summary.count>1}s{/if}
					</a>
					{*if $has_bought}
					<a class="wreviews" href="#product_reviews" data-uk-smooth-scroll>Write a comment</a>
					{/if*}
				</li>
				{/if}
				<li class="price-star fll mr20">
				...
				<div class="box-info-product">	
					<div class="product_attributes clearfix">
						...
						{if isset($productAddition['custom']) && $productAddition['custom']}
							<div class="custom-time uk-margin-small-top">Custom Time: <span>{$productAddition['custom']}</span></div>
						{/if}
