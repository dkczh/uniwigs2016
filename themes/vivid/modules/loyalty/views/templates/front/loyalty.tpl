...
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='Manage my account' mod='loyalty'}" rel="nofollow">{l s='My account' mod='loyalty'}</a>
<span class="navigation-pipe">{$navigationPipe}</span>
{if isset($ddate)}
<span class="navigation_page">{l s='Designer data' mod='loyalty'}</span>
{else}
<span class="navigation_page">{l s='My loyalty points' mod='loyalty'}</span>
{/if}
{/capture}
...
id="points" name="discount_name" style="
    width: 50px;
"><span>   input your point to transform voucher</span><span style="
    display: none;
" id="cuid">{$customerid}</span>
<span style="
    display: none;
" id="allpoints">{$totalPoints|intval}</span>


<button  class="uk-button uk-button-small" onclick='transform()'><span>transform</span></button>
<div id="result" style="
    width: 150px; 
"></div>
