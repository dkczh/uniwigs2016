...
{if isset($orderRet)}
	...
	<div class="box">
		...
		<ul>
			...
			<li>{l s='Please print out the'} <a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$orderRet->id|intval}")|escape:'html':'UTF-8'}" class="text-primary">{l s='PDF return slip'}</a> {l s='and include it with your package.'}</li>
			<li>{l s='Please see the PDF return slip'} (<a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$orderRet->id|intval}")|escape:'html':'UTF-8'}" class="text-primary">{l s='for the correct address.'}</a>)</li>
		</ul>
		{l s='When we receive your package, we will notify you by email. We will then begin processing order reimbursement.'}
		<br /><br /><a href="{$link->getPageLink('contact', true)|escape:'html':'UTF-8'}" class="text-primary">{l s='Please let us know if you have any questions.'}</a>
		<br />
