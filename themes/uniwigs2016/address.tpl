...
<div class="box">
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std uk-form uk-form-horizontal" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			{if $field_name eq 'company'}
				<div class="form-group uk-form-row">
					<label for="company" class="uk-form-label">{l s='Company'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
			...
			{if $field_name eq 'vat_number'}
				<div id="vat_area">
					<div id="vat_number">
						<div class="form-group uk-form-row">
							<label for="vat-number" class="uk-form-label">{l s='VAT number'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
							<div class="uk-form-controls">
			...
			<div class="required form-group dni uk-form-row">
			...
			{if $field_name eq 'address2'}
				<div class="required form-group uk-form-row">
					<label for="address2" class="uk-form-label">{l s='Address (Line 2)'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
					<div class="uk-form-controls">
			...
			{if $field_name eq 'Country:name' || $field_name eq 'country' || $field_name eq 'Country:iso_code'}
				<div class="required form-group uk-form-row">
					...
					<div class="uk-form-controls">
						<select id="id_country" class="form-control not_uniform" name="id_country">{$countries_list}</select>
					</div>
				</div>
			{/if}
			{if $field_name eq 'State:name'}
				{assign var="stateExist" value=true}
				<div class="required id_state form-group uk-form-row">
			...
			{if $field_name eq 'phone'}
				...
				<div class="form-group phone-number uk-form-row">
		...
		{/if}
		{if !$dniExist}
			<div class="required dni form-group unvisible uk-form-row">
				<label for="dni" class="uk-form-label">{l s='Identification number'} <sup>*</sup></label>
				<div class="uk-form-controls">
		...
		{if !$homePhoneExist}
			<div class="form-group phone-number uk-form-row">
				<label for="phone" class="uk-form-label">{l s='Home phone'}</label>
				<div class="uk-form-controls">
					<input class="{if isset($one_phone_at_least) && $one_phone_at_least}is_required{/if} validate form-control" data-validate="{$address_validation.phone.validate}" type="tel" id="phone" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone|escape:'html':'UTF-8'}{/if}{/if}"  />
				</div>
			</div>
		...
		<div class="required form-group uk-form-row" id="adress_alias">
			...
			<input type="text" id="alias" class="is_required validate form-control" data-validate="{$address_validation.alias.validate}" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{elseif isset($address->alias)}{$address->alias|escape:'html':'UTF-8'}{elseif !$select_address}{l s='My first address'}{/if}" />
		</div>
		<p class="submit2 uk-form-row">
			...
			<button type="submit" name="submitAddress" id="submitAddress" class="uk-button uk-button-primary">
				<span>
					{l s='Save'}
				</span>
			</button>
