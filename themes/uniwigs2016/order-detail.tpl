...
<div class="table_block">
	<table class="uk-table detail_step_by_step table table-bordered">
		<thead>
			<tr>
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				...
				<td>
					{if $state.ostate_name=='Payment accepted'||$state.ostate_name=='Preparation in progress'||$state.ostate_name=='Shipped'||$state.ostate_name=='Canceled'||$state.ostate_name=='Refund'}
					<span{if isset($state.color) && $state.color} style="background-color:{$state.color|escape:'html':'UTF-8'}; border-color:{$state.color|escape:'html':'UTF-8'};"{/if} class="label{if isset($state.color) && Tools::getBrightness($state.color) > 128} dark{/if}">{$state.ostate_name|escape:'html':'UTF-8'}
				   </span>
				   {else}
...
<div id="order-detail-content" class="table_block table-responsive">
	<table class="uk-table table table-bordered">
		<thead>
			<tr>
					...
					<tr class="item">
						...
						<td>
							...
							<div class="add_reviews">
								...
								<a href="#wrap_container" class="write_review uk-button uk-button-small" data-uk-modal oid="{$product.id_order}" iid="{$product.id_order_detail}" pid="{$product.product_id}" psku="{$product.product_reference}">Write a review</a>
								{/if}
...
{if $carriers|count > 0 && isset($carriers.0.carrier_name) && $carriers.0.carrier_name}
	<table class="uk-table table table-bordered footab">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
