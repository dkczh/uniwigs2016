...
<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
		<div class="form-group">
			<input class="form-control" type="email" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'|stripslashes}{/if}" placeholder="E-mail Address"/>
		</div>
		<p class="submit">
            <button type="submit" class="uk-button uk-button-primary"><span>{l s='Retrieve Password'}<i class="icon-chevron-right right"></i></span></button>
		</p>
	</fieldset>
	<ul class="clearfix footer_links">
		<li><a class="uk-button uk-button-small" href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow"><span><i class="icon-chevron-left"></i>{l s='Back to Login'}</span></a></li>
	</ul>
</form>
{/if}
</div>
