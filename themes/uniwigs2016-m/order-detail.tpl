...
<div class="box box-small clearfix">
	...
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
		...
		<a href="#" onclick="$(this).closest('form').submit(); return false;" class="uk-button"><span>{l s='Reorder'}</span></a>
	</form>
	{/if}
</div>
...
<div class="table_block">
	<table class="detail_step_by_step table table-bordered uk-table">
		<thead>
			<tr>
...
<div class="adresses_bloc">
	<div class="row">
		<div{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="address alternate_item box box-small">
				<li><div class="page-subheading">{l s='Delivery address'} ({$address_delivery->alias})</div></li>
		...
		<div>
			<ul class="address item {if $order->isVirtual()}full_width{/if} box box-small">
				<li><div class="page-subheading">{l s='Invoice address'} ({$address_invoice->alias})</div></li>
...
<div id="order-detail-content" class="table_block table-responsive uk-overflow-container">
...
{if $carriers|count > 0 && isset($carriers.0.carrier_name) && $carriers.0.carrier_name}
	...
	<table class="table table-bordered footab uk-table">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
...
{if !$is_guest}
	...
	{/if}
	{*<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std box" id="sendOrderMessage">
		...
		<p class="form-group uk-margin-top">
			<textarea class="form-control" cols="67" rows="3" name="msgText"></textarea>
		</p>
		<div class="submit uk-margin-top">
			<input type="hidden" name="id_order" value="{$order->id|intval}" />
			<input type="submit" class="unvisible" name="submitMessage" value="{l s='Send'}"/>
			<button type="submit" name="submitMessage" class="uk-button uk-button-primary"><span>{l s='Send'}<i class="icon-chevron-right right"></i></span></button>
		</div>
	</form>*}
