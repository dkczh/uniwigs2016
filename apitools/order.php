	...
	function out_item_order($db,$o_end,$o_begin)
	  {
		...
		$excefiled = array('id_order','product_reference','danwei','order_currency','product_quantity','product_price','item_total');
	 ...
	 function out_customer_order($db,$o_end,$o_begin){
		...
		$name = 'customer_order';
		// 执行productexcel 函数 前面 不允许有任何 echo 输出
		$excefiled = array('id_customer','email','address','recipient','country','state','city','postcode','phone','phone_mobile');
	...
	function out_fm_order($db,$o_end,$o_begin)
	  {
		...
		$sql = "SELECT 'Website' AS order_origin,o.id_order,o.id_customer,(
				...
				,cr.name AS order_carrier, cr.name AS order_carrier_real, o.shipping_number, o.payment, '' AS coupon,cu.iso_code AS order_currency
				...
				AND o.id_order IN (
				SELECT distinct id_order FROM `ps_order_history` WHERE id_order_state=4 AND `date_add`<'$o_end' and `date_add`>='$o_begin'
				)
				order by o.id_order" ;
