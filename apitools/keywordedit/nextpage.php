...
//echo $id ;
 //联合查询 表结构
$sql = " SELECT SQL_CALC_FOUND_ROWS * from  px_keyword LIMIT $id,10 ";
 ...
 foreach($result as $a){
	//$str.=$a['id_tag'];
	 $id = (int)$a['id_word'];
	 $keyword  = $a['keyword'];
	 $sku =substr($a['psku'],0,27); 
	 $pid =substr($a['pid'],0,27);
...
$str.=<<<EOT
	<tr>
      ...
      <td  onclick="document.location = '$curl/content.php?id=$id'"> $keyword</td>
      <td  onclick="document.location = '$curl/content.php?id=$id'">$sku</td>
	  <td  onclick="document.location = '$curl/content.php?id=$id'">$pid</td>
