...
class Ganalytics extends Module
{
	...
	public function hookFooter()
	{
		...
		$prds = $this->context->smarty->getTemplateVars('products');
		if (isset($prds[0]) && isset($prds[0][0]) && isset($prds[0][0]['id_product'])) {
			$prds_tmp = array();
			/*
			foreach ($prds as $product) {
				$prds_tmp = array_merge($prds_tmp, $product);
			}
			*/
			foreach ($prds as $product) {
				foreach ($product as $prd) {
					if (empty($prd['id_product'])) continue;
					$prds_tmp[] = $prd;
				}
			}
			$prds = $prds_tmp;
		}
		$products = $this->wrapProducts($prds, array(), true);
