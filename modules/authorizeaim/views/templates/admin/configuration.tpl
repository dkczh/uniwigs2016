		...
		<div class="margin-form">
			<select id="authorizeaim_hold_review_os" name="authorizeaim_hold_review_os">';
				...
				{foreach from=$order_states item='os'}
					{*<option value="{if $os.id_order_state|intval}" 
					{((int)$os.id_order_state == $AUTHORIZE_AIM_HOLD_REVIEW_OS)} 
					selected{/if}>
						{$os.name|stripslashes}
					</option>*}
					
					<option value="{$os.id_order_state}"
					{if ((int)$os.id_order_state == $AUTHORIZE_AIM_HOLD_REVIEW_OS)} 
					selected{/if}>
						{$os.name|stripslashes}
					</option>
				{/foreach}
