...
class LoyaltyDefaultModuleFrontController extends ModuleFrontController
{
	...
	public function processTransformPoints()
	{
		...
		$customer_points = (int)LoyaltyModule::getNewPointsByCustomer((int)$this->context->customer->id);
		...
		if ($customer_points >= 0)
		{
			...
			$now_points = Db::getInstance()->getValue('SELECT mark from px_customer_point  where id_customer ='.(int)$this->context->customer->id);
			$cart_rule->reduction_amount = LoyaltyModule::getVoucherValue((int)$now_points);
			...
			//如果 生成的折扣券 没有和积分里面的折扣券对应起来 择删除掉这张折扣券
	...
	{   
		//修改前 $customer_points = (int)LoyaltyModule::getPointsByCustomer((int)$this->context->customer->id);
		$customer_points = (int)LoyaltyModule::getNewPointsByCustomer((int)$this->context->customer->id);
		...
		//推送设计师数据到前台
		//判断当前客户是否为设计师
		if ((int)$this->context->customer->id_default_group == 4){	
	
		$ddate = LoyaltyModule::getDesignerData();
		
		//var_dump($ddate);
		$this->context->smarty->assign('ddate',$ddate);  
		$nowday = (string)date('Y-m-d');
		$frontday = (string)date("Y-m-d",strtotime("-7 day"));
		
		$this->context->smarty->assign('frontday',$frontday);
		$this->context->smarty->assign('nowday',$nowday);  
		...
		$nb_discounts = 0;
		$discounts = array();
		if ($ids_discount = LoyaltyModule::getNewDiscountByIdCustomer((int)$this->context->customer->id))
		{
