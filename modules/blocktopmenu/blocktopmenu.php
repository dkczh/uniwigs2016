...
class Blocktopmenu extends Module
{
    ...
    public function hookDisplayTop($param)
    {
        ...
        if (!$this->isCached('blocktopmenu.tpl', $this->getCacheId())) {
			...
			$this->getCatagory();//推送catagory 变量到 头部
	...
	public function getCatagory()
	{
			...
			$isexist = strstr($myurl, '/tag/'); 
			if($isexist){
			
			
			$tag_name =str_replace('/tag/','',$myurl);
			$tag_name  = str_ireplace(array('-','+'),' ',$tag_name);
			$catagory='';
			
			//str_ireplace(array('-','+'),' ',$url_key)
			
			$res= Db::getInstance()->getRow("select a.id_tag ,a.name,b.catagory from ps_tag a LEFT JOIN px_tag_extra b on a.id_tag =b.id_tag where a.`name` = '".$tag_name."'");
	...
	public function getProductCatagory()
	{
			...
			$myurl =  $_SERVER["REQUEST_URI"]; 
			//$myurl  = str_ireplace(array('-','+'),'  ',$myurl);

			$domain = strstr($myurl, "human-hair-wigs");
			$domain2= strstr($myurl, "synthetic-wigs"); 
			$domain3= strstr($myurl, "hair-extensions"); 
			$domain4= strstr($myurl, "hair-pieces");
			$domain5= strstr($myurl, "care-products"); 
			$domain6= strstr($myurl, "ponytails");
			$domain7= strstr($myurl, "bangs"); 
			$domain8= strstr($myurl, "half-wigs"); 
		    $domain9= strstr($myurl, "top-hairpieces");

            $domain10= strstr($myurl, "human-hair-lace-wigs");
            $domain11= strstr($myurl, "celebrity-hairstyles");
            $domain12= strstr($myurl, "full-lace-wigs");
            $domain13= strstr($myurl, "lace-front-wigs");
            $domain14= strstr($myurl, "glueless-full-lace-wigs");
            $domain15= strstr($myurl, "monofilament-wigs");
            $domain16= strstr($myurl, "silk-top-lace-wigs");
            $domain17= strstr($myurl, "jewish-wigs");

            $domain18= strstr($myurl, "classic-wigs");
            $domain19= strstr($myurl, "trendy-wigs");
            $domain20= strstr($myurl, "clearance-wigs");

            $domain21= strstr($myurl, "ombre-hair-extensions");

            $domain22= strstr($myurl, "mono-hair-pieces");

        
			if($domain!=null or $domain10!=null or $domain11!=null or $domain12!=null or $domain13!=null or $domain14!=null 
                or $domain15!=null or $domain16!=null or $domain17!=null){
			
			$this->smarty->assign('product_catagory',"Human Hair Wigs");
			}

			if($domain2!=null or $domain18!=null or $domain19!=null  or $domain20!=null){
		
			$this->smarty->assign('product_catagory',"Synthetic Wigs");
			}
			
			if($domain3!=null or $domain21!=null){
		
			$this->smarty->assign('product_catagory',"Hair Extensions");
			}
			if($domain4!=null or $domain6!=null or $domain7!=null or $domain8!=null or $domain9!=null or $domain22!=null){
		
			$this->smarty->assign('product_catagory',"Hair Pieces");
			}
			if($domain5!=null){
		
			$this->smarty->assign('product_catagory',"Care Products");
			}
