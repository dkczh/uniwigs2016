	    ...
	    if (! isdataempty) {
		    ...
		    $.ajax({
				...
				success:function(data){
				    ...
				    for(i in data){
						if (data[i].isvideo) {
							html += '<div class="box"><div class="hover-block"><div class="hover-bor"></div><div class="hover-op"></div></div><div class="pic"><a href="'+data[i].videosrc+'" data-uk-lightbox class="highslide"><img src="'+data[i].src+'"></a><em>VIDEO</em><p class="description">'+data[i].title+'</p><p class="name"><span class="name_icon"><i class="icon-login"></i></span><span>'+data[i].author_name+'</span></p><p class="view_btn"><a href="'+data[i].p_url+'" class="view_btn">VIEW PRODUCT ></a></p></div></div>';
						} else {
							//html += "<div class='box'><div class='pic'><a href='"+data[i].src+"' onclick='return hs.expand(this)' class='highslide'><img src='"+data[i].src+"'></a><p class='description'>"+data[i].title+"</p><p><a href='/index.php?controller=product&sku="+encodeURIComponent(data[i].sku)+"xxx"+encodeURIComponent(data[i].p_url)+"' class='view'>VIEW PRODUCT</a></p></div></div>";
							html += "<div class='box'><div class='hover-block'><div class='hover-bor'></div><div class='hover-op'></div></div><div class='pic'><a src='http://rvm.uniwigs.com/timthumb.php?src="+encodeURIComponent(data[i].src)+"' class='highslide' data-uk-lightbox><img src='http://rvm.uniwigs.com/timthumb.php?src="+encodeURIComponent(data[i].src)+"&w=300'></a><p class='description'>"+data[i].title+"</p><p class='name'><span class='name_icon'><i class='icon-user'></i></span><span>"+data[i].author_name+"</span></p><p class='view_btn'><a href='"+data[i].p_url+"' class='view_btn'>VIEW PRODUCT ></a></p></div></div>";
						}
				    }
				    render($(html));
				},
